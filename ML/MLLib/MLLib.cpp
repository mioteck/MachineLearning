...
namespace ML {
	...
	{
		Eigen::MatrixXd mat = ML::CreateMatrixEntries(entries, size, offset);
		int rows = size / offset;
		float* w = ML::InitWeight(offset + 1);

		int* perceptronResult = new int[rows];

		while (!ML::isClassified(perceptronResult, results, rows))
		{
			for (int i = 0; i < rows; i++)
			{
				perceptronResult[i] = ML::Perceptron(mat.row(i), w);
				if (perceptronResult[i] != results[i])
					ML::RuleRosenblatt(mat.row(i), w, perceptronResult[i], results[i], step);
			}
		}
	...
	float* ResolvePLA(float* entries, int size, int offset, int* results, float step)
	{
		...
		while (!ML::isClassified(perceptronResult, results, rows))
		{
			...
			perceptronResult[index] = ML::Perceptron(mat.row(index), w);
			if (perceptronResult[index] != results[index])
				ML::RulePLA(mat.row(index), w, results[index], step);
		}
	...
	{
		Eigen::MatrixXd r = ML::CreateMatrix(results, size / offset, 1);
		Eigen::MatrixXd mat = ML::CreateMatrixEntries(entries, size, offset);
